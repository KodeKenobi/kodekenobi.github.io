...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const eventRemovers: (() => void)[] = [];

    // Wrap updateFrame to store the frame id
    function updateFrameWrapper() {
      updateFrame();
      animationFrameId = requestAnimationFrame(updateFrameWrapper);
    }

    // Replace direct requestAnimationFrame call with wrapper
    function startAnimation() {
      animationFrameId = requestAnimationFrame(updateFrameWrapper);
    }

    // Replace all event listener additions with tracked removers
    ...
    const mousedownHandler = (e: Event) => {
      ...
      const posX = scaleByPixelRatio(evt.clientX);
      const posY = scaleByPixelRatio(evt.clientY);
    ...
    // first mousemove
    const handleFirstMouseMove = (e: Event) => {
      const evt = e as MouseEvent;
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(evt.clientX);
      const posY = scaleByPixelRatio(evt.clientY);
      ...
      document.body.removeEventListener(
        "mousemove",
        handleFirstMouseMove as EventListener
      );
    };
    addTrackedEventListener(
      document.body,
    ...
    // first touchstart
    const handleFirstTouchStart = (e: Event) => {
      ...
      document.body.removeEventListener(
        "touchstart",
        handleFirstTouchStart as EventListener
      );
    };
    addTrackedEventListener(
      document.body,
    ...
    // (handled in first mousemove/touchstart)
  ...
  return (
    <div className="fixed top-0 left-0 z-50 pointer-events-none w-full h-[100dvh]">
      <canvas
        ref={canvasRef}
        id="fluid"
